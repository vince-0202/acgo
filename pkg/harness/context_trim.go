...
func (cc *ContextController) optionsOrDefault() *ContextOptions {
...
func (cc *ContextController) hasCompactLLM() bool {
	if cc == nil || cc.compactProvider == nil {
		return false
	}
	return strings.TrimSpace(cc.compactModel.Provider) != "" && strings.TrimSpace(cc.compactModel.ID) != ""
}
...
func (cc *ContextController) shouldAutoCompact(opts *ContextOptions) bool {
	if !cc.hasCompactLLM() {
		return false
	}
...
func (cc *ContextController) applyDeterministicTrim(opts *ContextOptions) {
	if cc == nil || len(cc.Messages) == 0 {
		return
	}
	...
	if len(turns) == 0 {
		cc.Messages = append(append([]communi.Message(nil), prefix...), body...)
		return
	}
	...
	cc.Messages = append(append([]communi.Message(nil), prefix...), out...)
}

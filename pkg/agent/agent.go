...
// PromptScheduledTask runs one turn triggered by a scheduled job.
// The task is sent as a hidden user message so UIs can suppress transcript echo.
...
func (a *Agent) promptWithInitialMessage(ctx context.Context, initialMsg communi.Message) error {
	...
	agentTurnErr := a.turnManager.startAgentTurn(ctx, a, &initialMsg)
	a.emit(NewEvent(
		WithEventType(EventAgentEnd),
		WithEventAgent(a),
		WithEventError(agentTurnErr),
	))
	if agentTurnErr == nil {
		return nil
	}
...
func (a *Agent) ensureSystemPromptMessage() {
	...
	if len(msgs) > 0 && msgs[0].Role == keys.AgentRoleSystem {
		...
		a.context.ReplaceMessages(msgs)
		return
	}

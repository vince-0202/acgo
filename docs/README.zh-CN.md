...
- **Harness 代理层**：将上下文、权限、memory、skills、sub-agent 等 controller 独立装配到 Agent 上，并作为底层 Agent 的执行代理。

### 快速开始

以下步骤介绍如何使用 acgo 创建一个 agent、显式构建 harness、将 harness 附着到 agent，并通过 harness 代理完成一轮交互。代码见 [`example/prompt_one_with_settings.go`](/Users/wangsj/workspace/acgo/example/prompt_one_with_settings.go)。
...
- `h.Prompt(...)` / `h.Subscribe(...)`：通过 harness 代理执行，确保 harness 约束生效

默认 TUI 也遵循这条路径：先构建 agent，再构建默认 harness，attach 之后通过 harness 作为公开执行入口。
